...
with __st.echo(), streambook.st_stdout('info'):
    ...
    model
__st.markdown(r"""Initialization is straightforward. The only thing you need to do is to call `init` on your module with a random key:""", unsafe_allow_html=True)
with __st.echo(), streambook.st_stdout('info'):
    import jax

    model = model.init(key=42)
    model
__st.markdown(r"""Now we will be reusing the previous NoisyLinear model, and we will create an optax optimizer that is used to train the model:""", unsafe_allow_html=True)
with __st.echo(), streambook.st_stdout('info'):
    import optax
...
__st.markdown(r"""Before we start training lets get some data:""", unsafe_allow_html=True)
with __st.echo(), streambook.st_stdout('info'):
    import numpy as np
...
with __st.echo(), streambook.st_stdout('info'):
    ...
    plt.show()
    fig  # __st
__st.markdown(r"""We can see that the model has learned the general trend, but because of the `NoisyLinear` modules we have a bit of noise in the predictions.""", unsafe_allow_html=True)
