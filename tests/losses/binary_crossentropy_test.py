...
def test_compatibility():
    ...
    bce_elegy = tx.losses.BinaryCrossentropy()
    bce_tfk = tfk.losses.BinaryCrossentropy()
    assert np.isclose(bce_elegy(y_true, y_pred), bce_tfk(y_true, y_pred), rtol=0.0001)

    # Standard BCE, considering prediction tensor as logits
    y_logits = jnp.log(y_pred) - jnp.log(1 - y_pred)
    bce_elegy = tx.losses.BinaryCrossentropy(from_logits=True)
    ...
    bce_elegy = tx.losses.BinaryCrossentropy(reduction=tx.losses.Reduction.SUM)
    bce_tfk = tfk.losses.BinaryCrossentropy(reduction=tfk.losses.Reduction.SUM)
    assert np.isclose(bce_elegy(y_true, y_pred), bce_tfk(y_true, y_pred), rtol=0.0001)

    # BCE with reduction method: NONE
    bce_elegy = tx.losses.BinaryCrossentropy(reduction=tx.losses.Reduction.NONE)
    bce_tfk = tfk.losses.BinaryCrossentropy(reduction=tfk.losses.Reduction.NONE)
    assert jnp.all(
        np.isclose(bce_elegy(y_true, y_pred), bce_tfk(y_true, y_pred), rtol=0.0001)
    )

    # BCE with label smoothing
    bce_elegy = tx.losses.BinaryCrossentropy(label_smoothing=0.9)
