...
class TestTreex:
    ...
    def test_preserve_ids(self):
        ...
        @tx.experimental_preserve_ids_io(jax.jit)
        def f(m1: SomeModule, m2: SomeModule):
            assert m1 is m2

            m1.x = 2
            m3, m4 = tx.experimental_preserve_ids(jax.tree_map)(lambda x: x, (m1, m2))
            m5, m6 = tx.experimental_preserve_ids(jax.tree_map)(lambda x: x, (m1, m2))

            assert m3 is m4
            assert m5 is m6
            assert m1 is not m3 and m2 is not m4
            assert m3 is not m5 and m4 is not m6
    ...
    def test_preserve_ids_optimizer(self):
        class Child(tx.Module):
            x: tx.Parameter[float, jnp.ndarray]
        ...
        @tx.experimental_preserve_ids_io(jax.grad)
        def grad_fn(m: Parent):
            return sum(jnp.mean(x) for x in jax.tree_leaves(m.parameters()))

        m = tx.experimental_preserve_ids(Parent(Child(1.0)).init)(42)
        params = m.parameters()

        optimizer = tx.Optimizer(optax.adam(3e-4)).init(params)

        grads: Parent = grad_fn(m)

        assert grads.left is grads.right

        params = tx.experimental_preserve_ids(optimizer.apply_updates)(grads, params)

        assert params.left is params.right
