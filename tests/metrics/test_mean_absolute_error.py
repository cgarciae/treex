...
class TestMAE:
    ...
    def test_mae_weights_batch_dim(self, use_sample_weight):
        ...
        mae_tx = tx.metrics.MeanAbsoluteError().reset()
        if use_sample_weight:
    ...
    def test_mae_weights_values_dim(self, use_sample_weight):
        ...
        mae_tx, _ = tx.metrics.MeanAbsoluteError().reset()(**params)
    ...
    def test_mae_short(self):
        ...
        mae_tx_long, _ = tx.metrics.MeanAbsoluteError().reset()(
            target=target, preds=preds
        )
        mae_tx_short, _ = tx.metrics.MAE().reset()(target=target, preds=preds)
